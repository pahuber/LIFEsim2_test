...
def create_blackbody_spectrum(temperature,
                              ...
                              maximum_angle_per_pixel: astropy.units.Quantity) -> np.ndarray:
    ...
    :param maximum_angle_per_pixel: Maximum angle per grid pixel of the sky coordinates map
    ...
    return convert_blackbody_to_flux(blackbody_spectrum_binned, maximum_angle_per_pixel, wavelength_bin_centers)


def convert_blackbody_to_flux(blackbody_spectrum_binned: np.ndarray,
                              maximum_angle_per_pixel: astropy.units.Quantity,
    ...
    # calc solid angle of source
    solid_angle = np.pi * (((1 * u.Rearth).to(u.m) / (10 * u.pc).to(u.m)) * u.rad) ** 2

    for index in range(len(blackbody_spectrum_binned)):
        current_flux = (blackbody_spectrum_binned[index] * (solid_angle).to(u.sr)).to(
            u.ph / u.m ** 2 / u.s / u.um,

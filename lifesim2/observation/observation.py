...
class Observation():
    ...
    def _set_from_config(self):
        ...
        self.field_of_view = self._config_dict['observation']['field_of_view']
        self.integration_time = self._config_dict['observation']['integration_time']
        self.modulation_period = self._config_dict['observatory']['instrument_parameters']['modulation_period']
    ...
    def create_observatory(self):
        ...
        self.observatory._set_from_config(observatory_dictionary=self._config_dict['observatory'])
    ...
    def run(self):
        ...
        for time_index, time in enumerate(self.time_range):
            ...
            # 
            # plt.imshow(np.real(intensity_response_vector[2] - intensity_response_vector[3]))
            # plt.colorbar()
    ...
    def get_perturbation_matrix(self) -> np.ndarray:
        ...
        for index in range(self.observatory.beam_combination_scheme.number_of_inputs):
            diagonal_of_matrix[index] = np.random.uniform(0.2, 0.4) * np.exp(1j * np.random.uniform(-0.4, 0.4))
